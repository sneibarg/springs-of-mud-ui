...
class MenuList:
    """
    Pure rendering for MenuBar + MenuDropdown.
    Owns all Rect/TextField drawing.
    """
    ...
    def draw_bar(self, *, bar: Rect) -> None:
        bar.fill(self.m.bar_bg)

    def draw_label(self, *, menu: MenuDropdown, bar_y: int, bar_h: int, active: bool) -> None:
        w = self.label_width(menu.label)
        if active:
            Rect(menu.x, bar_y, w, bar_h).fill(self.m.bar_active_bg)
        self.text.draw_text(x=menu.x + self.m.pad_x, y=bar_y + self.m.bar_text_y, text=menu.label, col=self.m.text_col)

    def draw_dropdown(self, *, menu: MenuDropdown, hovered_idx: Optional[int]) -> None:
        if not menu.items:
            return

        panel = self.dropdown_rect(menu)
        panel.fill(self.m.dropdown_bg)
        panel.border(self.m.dropdown_border)

        yy = panel.y + self.m.dropdown_pad_y
        for idx, it in enumerate(menu.items):
            if hovered_idx == idx:
                Rect(panel.x + 1, yy, panel.w - 2, self.m.item_h - 1).fill(self.m.item_hover_bg)
            self.text.draw_text(x=panel.x + self.m.pad_x, y=yy + self.m.item_text_y, text=it.label, col=self.m.text_col)
            yy += self.m.item_h
