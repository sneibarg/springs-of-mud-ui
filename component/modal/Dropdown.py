...
class Dropdown:
    ...
    def draw(self, ctx, label: Optional[str] = None) -> None:
        self.r.draw_dropdown_field(ctx, self.rect, label=label, value=str(self.value), open_=self.open)

    def draw_popup(self, ctx) -> None:
        if not self.open:
            return
        pr = self.popup_rect()
        self.r.draw_dropdown_popup(ctx, pr, options=self.options, selected=str(self.value), row_h=self.rect.h)
