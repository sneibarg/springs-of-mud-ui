...
class TextField:
    """
    Centralizes text rendering.

    IMPORTANT: All rendering goes through ctx:
      - ctx.gfx.text(x, y, text, col)
      - Rect(...).fill(ctx, col)
    """
    ...
    def draw_text(self, ctx, *, x: int, y: int, text: str, col: int | None = None, scale: int | None = None) -> None:
        c = self.s.col if col is None else col
        sc = self.s.scale if scale is None else scale

        if sc <= 1:
            ctx.gfx.text(x, y, text, c)
            return

        for dx in range(sc):
            for dy in range(sc):
                ctx.gfx.text(x + dx, y + dy, text, c)
    ...
    def draw_lines(
        ...
        *,
        ...
        for line in lines:
            for dx in range(sc):
                for dy in range(sc):
                    ctx.gfx.text(x + dx, yy + dy, line, c)
            yy += line_h * sc
    ...
    def draw_input(
        ...
        *,
        ...
        self.draw_text(ctx, x=rect.x + pad, y=rect.y + text_y_offset, text=prompt, col=prompt_col)
        ...
        if has_selection and selection_start is not None and selection_end is not None:
            ...
            if visible_sel_start < len(visible_text) and visible_sel_end > 0:
                ...
                Rect(sx, rect.y + text_y_offset, sw, selection_h).fill(ctx, selection_col)

        tx = rect.x + pad + prompt_w_px
        self.draw_text(ctx, x=tx, y=rect.y + text_y_offset, text=visible_text, col=text_col)

        if blink_on:
            visible_cursor = input_cursor - view_start
            cx = rect.x + pad + prompt_w_px + visible_cursor * self.s.char_w
            Rect(cx, rect.y + caret_y_offset, caret_w, caret_h).fill(ctx, caret_col)
